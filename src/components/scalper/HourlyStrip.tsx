...
export default function HourlyStrip({ hourly, now, timeframe, symbol }: HourlyStripProps) {
  ...
  /* Trading-hour count is data-driven: gold/NAS100/GER40 have a 00:xx
     session break (23 trading hours), US30 + the FX markets trade all 24.
     The title and a11y copy follow the JSON, not a hardcoded 23. */
  const breakHours = hourly.hours.filter((h) => h.avg_range_atr == null || h.bar_count === 0).map((h) => h.hour_utc)
  const tradingHours = hourly.hours.length - breakHours.length
    ...
    <section aria-label="Hourly volatility overview">
      <div className="panel p-4 sm:p-5">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="panel-title">Hourly Overview — {tradingHours} trading hours</h2>
        ...
        <div ref={containerRef} className="relative mt-4">
          <div className="flex items-end gap-[3px]" style={{ height: STRIP_H }} role="img"
            aria-label={`Bar chart of average ${timeframe} range in ATR by ${tzSuffix(tz)} hour.${breakHours.length > 0 ? ` Hour ${utcHourInTz(0, tz, now)} is hollow: session break.` : ' All 24 hours trade — no session break.'}`}>
            {hourly.hours.map((h) => {
            ...
            {hoverRow ? (
              hoverRow.avg_range_atr == null || hoverRow.bar_count === 0 ? (
                ...
                <span className="text-text1">
                  ...
                  {fmtSlotRange(hoverRow.avg_range_price, symbol)}
